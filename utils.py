...
from more_dataset import Aircraft, Cub2011, Dogs, bFFHQ
...
def get_dataset(dataset, data_path, batch_size=1, subset=None, subset_size=None, epc=None, args=None, set_seed=True):
    ...
    if dataset == 'CIFAR10':
        ...
        #dst_val = torchvision_datasets.Flowers102(data_path, split = 'val', download=True, transform=transform)
        dst_test = torchvision_datasets.Flowers102(data_path, split = 'test', download=True, transform=transform)
        # class_names = dst_train.classes
        # class_map = {x: x for x in range(num_classes)}
    
    elif dataset.startswith('urbancars'):
        ...
        dst_train = UrbanCars(root="/data/urbancars", split= 'train', spurious_label_type=UrbanCarsSpuriousLabel.BOTH, verbose=True, transform=transform)
        dst_train.initialize()
        dst_test = UrbanCars(root="/data/urbancars", split= 'test', spurious_label_type=UrbanCarsSpuriousLabel.BOTH, verbose=True, transform=transform)
        dst_test.initialize()

    

    elif dataset.startswith('waterbirds'):
        ...
        dataset = wilds_get_dataset(dataset="waterbirds", download=True, root_dir="/data")

        dst_train = dataset.get_subset('train', transform=transform)
        dst_train = WILDSDatasetWrapper(dataset=dst_train, metadata_spurious_label="background", verbose=True)
        dst_test = dataset.get_subset('test', transform=transform)
        dst_test = WILDSDatasetWrapper(dataset=dst_test, metadata_spurious_label="background", verbose=True)

    
    elif dataset.startswith('bffhq'):
        ...
        dst_train = bFFHQ(root="/data", split="train", transform=transform)
        dst_test = bFFHQ(root="/data", split="test", transform=transform)
        ...
        dataset = wilds_get_dataset(dataset="celebA", root_dir="/home/data")

        dst_train = dataset.get_subset(
                "train",
                transform=transform
            )
        
        dst_train = WILDSDatasetWrapper(dataset=dst_train, metadata_spurious_label="male", verbose=True)
        

        dst_test = dataset.get_subset(
            "test",
            transform=transform
        )
        dst_test = WILDSDatasetWrapper(dataset=dst_test, metadata_spurious_label="male", verbose=True)
        ...
        dataset = wilds_get_dataset(dataset='camelyon17', download=False, root_dir="/home/data")
        dst_train = dataset.get_subset(
                "train",
                transform=transform
            )
        dst_train = WILDSDatasetWrapper(dataset=dst_train, metadata_spurious_label="hospital", verbose=True)
        dst_test = dataset.get_subset(
            "test",
            transform=transform
        )

        dst_test = WILDSDatasetWrapper(dataset=dst_test, metadata_spurious_label="hospital", verbose=True)
        dst_val = dataset.get_subset(
            "val",
            transform=transform
        )
        dst_val = WILDSDatasetWrapper(dataset=dst_val, metadata_spurious_label="hospital", verbose=True)
        ...
        hparams = hparams_registry.default_hparams('', 'terra_incognita')
 
        train_domain = ["L100", "L38", "L43"]
        test_domain = ["L46"]
        domainbed_dataset = datasets.TerraIncognita
        ...
        from domainbed import datasets
        from domainbed import hparams_registry
        hparams = hparams_registry.default_hparams('', 'office_home')
        domainbed_dataset = datasets.OfficeHome
        train_domain = ['A', 'C', 'P']
        test_domain = ['R']
        ...
        from domainbed import datasets
        from domainbed import hparams_registry
        hparams = hparams_registry.default_hparams('', 'VLCS')
        domainbed_dataset = datasets.VLCS
        train_domain = ['V', 'L', 'C']
        test_domain = ['S']
        ...
        from domainbed import datasets
        from domainbed import hparams_registry
        hparams = hparams_registry.default_hparams('', 'PACS')
        domainbed_dataset = datasets.PACS
        train_domain = ['P', 'A', 'C']
        test_domain = ['S']
    else:
        exit('unknown dataset: %s'%dataset)

    # Processing for domainbed dataset
    if dataset in ["terra_incognita", "office", "VLCS", "PACS"]:
        dst_train = ConcatDataset(domainbed_dataset("/data/domainbed/", train_domain, hparams).datasets)
        dst_test = ConcatDataset(domainbed_dataset("/data/domainbed/", test_domain, hparams).datasets)
